...
int main()
{
    ...
    int a[9][9][10];srand((unsigned)time(NULL));
   qoop:  initializecube(a);contra=1;
									 printf("\n\n\n\n");/*print(a);a[0][6][0]=4;bakline(a,0,6,4);
...
{ 
  //	if(m==14)return 0;
  	fprintf(touche,"%d %d\t",contra,m);
	  contra++;
  	//if(m>loolchand)loolchand=m;
	  //printf("\n\n\n\n%d",loolchand);printarray;//goto qoop;//printf("pichpichpicpichthoo");
...
/*    printf("jio%dhhjjh %dhjkhjk\n",r,c);*/
  ...
  int ch[z],yoo;i=1;//for(yoo=1;yoo<=z;yoo++)ch[yoo]=1;
  for(k=1;k<=9;k++)
  if(a[x][y][k][m][0]==1){ch[i]=k;/*printf("/n%d \n",k);*/i++;}
    ...
    int d1, d2,d;
    for(k=1;k<=z;k++){  if(ch[k]>9)printf("ye kya ho gya %d\n\n\n",contra);
                        d1=(rand()%z)+1;
                        d2=(rand()%z)+1;
                        c=ch[d1];
                        ch[d1]=ch[d2];
...
;                                if(l==13)return 13;//if(l==0)return 0;
                                }
                   if(u==1){/*printf("batch\n");*/printf("\n\ndifficulty index = %d\n\n\n\n\nSolved sudoku \n\n",m);printarray(v);return 13;}                
                    fprintf(touche,"%d %d\t",contra,m);  contra++;fprintf(touche,"\n\n"); printarray(v) ;   
                            }
