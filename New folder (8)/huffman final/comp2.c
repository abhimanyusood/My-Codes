         ...
         while(1){
                  ...
                  for(x=0;x<COUNT;x++)if(cha==symbol[x])break;
                         for(i=0;;){ 
                                ...
                                //manipulate nth bit of octa with code[x][i]
                                n++;i++;//printf("\n\nboot %d\n",n); 
                                if(n==8){n=0;octa=add(octa);fprintf(z,"%c",octa);printf("CHIKI%c  ",octa);k++;/*printf("pp %c pp\n ",octa);display(octa);printf("pp\n ",octa);*/octa=octa&~octa;/*parse octa;initialize octa*/}
                                if(code[x][i]=='\0'){octa=add(octa);fprintf(z,"%c",octa);printf("LOOPI%c  ",octa);break;}
                                }
                  //printf("tatti\n"); 
